...
//===================================================================
// CAANewVersionCmd.cpp
// Header definition of class CAANewVersionCmd
//===================================================================
//
// Usage notes:
//
...
#include "CAANewVersionCmd.h"
#include "CATCreateExternalObject.h"
CATCreateClass(CAANewVersionCmd);
...
//-----------------------------------------------------------------------------
CAANewVersionCmd::CAANewVersionCmd() :
	CATStateCommand("CAANewVersionCmd", CATDlgEngOneShot, CATCommandModeShared)
	, _OKAgent(NULL),
	_ApplyAgent(NULL),
	_CloseAgent(NULL)
{
	...
	_Panel = new CAANewVersionDlg(pWnd, "Insert FO & instruction");
	_Panel->Build();
...
void CAANewVersionCmd::BuildGraph()
{
	...
	AddTransition(pSelectionState, NULL, AndCondition(IsOutputSetCondition(_OKAgent), Condition((ConditionMethod)&CAANewVersionCmd::OKAction)), NULL);
	//AddTransition(pSelectionState, NULL, IsOutputSetCondition(_OKAgent), Action((ActionMethod)&CAANewVersionCmd::OKAction));

	// Transition from Input state to NULL, when click on Cancel button
	AddTransition(pSelectionState, pSelectionState, IsOutputSetCondition(_ApplyAgent), Action((ActionMethod)&CAANewVersionCmd::ApplyAction));

	// Transition from Input state to NULL, when click on Close button
	AddTransition(pSelectionState, NULL, IsOutputSetCondition(_CloseAgent), Action((ActionMethod)&CAANewVersionCmd::CancelAction));

	AddTransition(pSelectionState, NULL, IsOutputSetCondition(_CancleAgent), Action((ActionMethod)&CAANewVersionCmd::CancelAction));

	AddTransition(pSelectionState, pSelectionState, IsOutputSetCondition(_pSelectAgent), Action((ActionMethod)&CAANewVersionCmd::ElementSelection, NULL, NULL, (void*)1));
...
// ActionOne ()
//-------------------------------------------------------------------------
CATBoolean CAANewVersionCmd::ActionOne(void *data)
{
...
CATBoolean CAANewVersionCmd::ElementSelection(void * data)
{
...
void CAANewVersionCmd::FilterSelectObject()
{
	...
	if ((sSelectObjectType == TYPE_Workplan) || (sSelectObjectType == TYPE_CUS_FO_Workplan) /*|| (sSelectObjectType == TYPE_Product)*/ )					// workplan
	{
		...
		pEditor->SetText("CAACAABatchNewVersion PLM1 https://int-plm.CAA.intra:443/3dspace ODREMTlBRUMyMUE4NEU3REI1N0IzMjEyMzk3MDQwMDl8NTAwNzgwfDUwMDc4MHx8fDB8 "
...
CATBoolean CAANewVersionCmd::ApplyAction(void *data)
{
	if (_ApplyAgent)
	{
...
CATBoolean CAANewVersionCmd::OKAction(void *data)
{
	if (_OKAgent)
	{
...
CATBoolean CAANewVersionCmd::CheckSelectObject()
{
...
CATBoolean CAANewVersionCmd::CancelAction(void *data)
{
...
void CAANewVersionCmd::RunNewVersion()
{
