...
#include "CATAdpPLMAttributeSet.h"
#include "CATIAdpPLMValueReader.h"
//DELPPRSystemItf 
...
CATIPLMComponent_var CUSCAAUtilService::GetCurrentPPRComp()
{
	CATIPLMComponent_var spPPRComp = NULL_var;
	CATFrmEditor* pFrmEdt = CATFrmEditor::GetCurrentEditor();
	CATListPtrCATIPLMComponent Roots;
	CATPLMComponentInterfacesServices::GetEditedRootPLMComponents(pFrmEdt, Roots);
	if (Roots.Size() != 0)
	{
		spPPRComp = Roots[1];
	}
...
CATLISTV(CATBaseUnknown_var) CUSCAAUtilService::GetCurrentPPRSList(unsigned long iType)
{
	CATLISTV(CATBaseUnknown_var) listPPRS;
	DELIPPRUIServices_var spPPRUIService;
	HRESULT hr = GetPPRUIServices(spPPRUIService);
	if (hr == S_OK && spPPRUIService != NULL_var)
	{
		spPPRUIService->GetPPRRootsFromCurrentEditor(listPPRS, iType);
	}
	return listPPRS;
}
...
void StartBatchCmd(CATUnicodeString cmdLine)
{
	int result = -1;
	...
	const CATUnicodeString iUrl, const CATUnicodeString iHttpType, const char *iData)
{
	...
	DWORD dwOpenRequestFlags = INTERNET_FLAG_SECURE | INTERNET_FLAG_RELOAD;
...
HRESULT CUSCAAUtilService::CopyDictionariesModel(CATBaseUnknown * ipSourceReference, CATBaseUnknown * opTargetReference)
{
	...
	hr = piCfgEntity->GetDictionaries(out_listDictionaries);
	_RELEASE_PTR_(piCfgEntity);
	int n_model = out_listDictionaries.Size();
	if (n_model <= 0)
		return S_FALSE;
	...
	for (int i = 1; i <= n_model; i++)
	{
		...
		if (spConfigDic == NULL)
			continue;*/
		hr = spConfigAuthoring->AttachDictionary(*out_listDictionaries[i]);
	}
	...
	out_listDictionaries.RemoveAll();
...
bool CUSCAAUtilService::CheckEffectivity(const CATUnicodeString iEffectivity)
{
	...
	std::string pattern{ "2\\d{4}((-2\\d{4})?|\\+)(,2\\d{4}((-2\\d{4})?|\\+))*" }; // 20101-20105,20107,20108-29999
...
bool CUSCAAUtilService::CheckEffectivityRange(const CATUnicodeString iFatherEffectivity,const CATUnicodeString iEffectivity)
{
	bool IsFormat = true;

	CATListOfCATUnicodeString fatherEffectList = GetListStringValue(iFatherEffectivity, ",");
	CATListOfInt n_FatherEffects;
	for (int i = 1; i <= fatherEffectList.Size(); ++i)
	{
		CATUnicodeString fatherEffectRange = fatherEffectList[i];
		if (fatherEffectRange.SearchSubString("+") != -1)
		{
			fatherEffectRange.ReplaceSubString("+","");
			int n_Effect = 0;
			fatherEffectRange.ConvertToNum(&n_Effect);
			for (int j = n_Effect; j <= 29999; ++j)
			{
				n_FatherEffects.Append(j);
			}
			
		}
		else
		{
			CATListOfCATUnicodeString fatherEffectRangeList = GetListStringValue(fatherEffectRange, "-");
			if (fatherEffectRangeList.Size() == 1)
			{
				int n_Effect = 0;
				fatherEffectRangeList[1].ConvertToNum(&n_Effect);
				n_FatherEffects.Append(n_Effect);
			}
			else
			{
				int n_Effect1 = 0;
				fatherEffectRangeList[1].ConvertToNum(&n_Effect1);
				int n_Effect2 = 0;
				fatherEffectRangeList[2].ConvertToNum(&n_Effect2);
				for (int j = n_Effect1; j <= n_Effect2; ++j)
				{
					n_FatherEffects.Append(j);
				}
			}
	...
	CATListOfCATUnicodeString EffectList = GetListStringValue(iEffectivity, ",");
	for (int i = 1; i <= EffectList.Size(); ++i)
	{
		CATUnicodeString EffectRange = EffectList[i];
		if (EffectRange.SearchSubString("+") != -1)
		{
			EffectRange.ReplaceSubString("+", "");
			int n_Effect = 0;
			EffectRange.ConvertToNum(&n_Effect);
			for (int j = n_Effect; j <= 29999; ++j)
			{
				if (n_FatherEffects.Locate(j) == 0)
				{
					return false;
				}
			}
		...
		{
			CATListOfCATUnicodeString EffectRangeList = GetListStringValue(EffectRange, "-");
			if (EffectRangeList.Size() == 1)
			{
				int n_Effect = 0;
				EffectRangeList[1].ConvertToNum(&n_Effect);
				if (n_FatherEffects.Locate(n_Effect) == 0)
				{
					return false;
				}
			}
			...
			{
				int n_Effect1 = 0;
				EffectRangeList[1].ConvertToNum(&n_Effect1);
				int n_Effect2 = 0;
				EffectRangeList[2].ConvertToNum(&n_Effect2);
				for (int j = n_Effect1; j <= n_Effect2; ++j)
				{
					if (n_FatherEffects.Locate(j) == 0)
					{
...
bool CUSCAAUtilService::CheckEffectivityIntersection(const CATUnicodeString iEffectivity, const CATUnicodeString iOtherEffectivity)
{
	bool IsFormat = false;

	CATListOfCATUnicodeString otherEffectList = GetListStringValue(iOtherEffectivity, ",");
	CATListOfInt n_otherEffects;
	for (int i = 1; i <= otherEffectList.Size(); ++i)
	{
		CATUnicodeString otherEffectRange = otherEffectList[i];
		if (otherEffectRange.SearchSubString("+") != -1)
		{
			otherEffectRange.ReplaceSubString("+", "");
			int n_Effect = 0;
			otherEffectRange.ConvertToNum(&n_Effect);
			for (int j = n_Effect; j <= 29999; ++j)
			{
				n_otherEffects.Append(j);
			}

		}
		else
		{
			CATListOfCATUnicodeString otherEffectRangeList = GetListStringValue(otherEffectRange, "-");
			if (otherEffectRangeList.Size() == 1)
			{
				int n_Effect = 0;
				otherEffectRangeList[1].ConvertToNum(&n_Effect);
				n_otherEffects.Append(n_Effect);
			}
			else
			{
				int n_Effect1 = 0;
				otherEffectRangeList[1].ConvertToNum(&n_Effect1);
				int n_Effect2 = 0;
				otherEffectRangeList[2].ConvertToNum(&n_Effect2);
				for (int j = n_Effect1; j <= n_Effect2; ++j)
				{
					n_otherEffects.Append(j);
				}
	...
	for (int i = 1; i <= EffectList.Size(); ++i)
	{
		...
		else
		{
			...
			if (EffectRangeList.Size() == 1)
			{
				int n_Effect = 0;
				EffectRangeList[1].ConvertToNum(&n_Effect);
				if (n_otherEffects.Locate(n_Effect) != 0)
				{
					return true;
				}
			}
...
void CUSCAAUtilService::AddExtensionByEKL(CATIPLMNavReference_var& spObjectRef, CATUnicodeString iExtensionName)
{
	CATBaseUnknown_var spObject(spObjectRef);
	CATICkeParmFactory_var spVolFactory = CATCkeGlobalFunctions::GetVolatileFactory();
	CATICkeParm_var spKweParmFrom = spVolFactory->CreateObjectReference(spObject);

	CATCkeListOfParm iListOfParameters = new CATListValCATBaseUnknown_var();
	CATCkeEvalContext *pEC = CATCkeGlobalFunctions::GetFunctionFactory()->CreateEvaluationContext(NULL_var);
	iListOfParameters->Append(spKweParmFrom);

	CATICkeParm_var spExtensionName = NULL_var;
	spExtensionName = spVolFactory->CreateString("ExtensionName", iExtensionName);
	iListOfParameters->Append(spExtensionName);

	CATIPLMComponent_var ospPLMComp = NULL_var;
	CATUnicodeString ResourceId = "KnowledgeResources|AddExtension";
	...
	CATBaseUnknown_var spCatalogAsCBU;
	setupDataFinder->FindPointedResource(ResourceId, NULL_var, spCatalogAsCBU);
	
	if (spCatalogAsCBU != NULL_var)
	{
		CATICkeFunction_var spFunc = spCatalogAsCBU;

		spFunc->Run(iListOfParameters, NULL_var, pEC);
		if (pEC)
		{
			delete pEC;
			pEC = NULL;
	...
	PLMSetupDataGlobalFunctions::RemovePLMSetupDataFinder(SetupDataFinderID);
}
...
HRESULT CUSCAAUtilService::GetPLMAttributeValue(CATIAdpPLMIdentificator* pObjAdpIdent, const CATUnicodeString& iStrPLMType, const CATUnicodeString& iStrAttrName, CATUnicodeString& oStrAttrValue)
{
	HRESULT hr = E_INVALIDARG;
	CATAdpPLMComponentUniqueKey UniqueKey;
	hr = CATAdpPLMQueryServices::GetUniqueKey(pObjAdpIdent, UniqueKey);
	if (hr != S_OK)
	{
		return hr;
	}
	CATAdpPLMComponentInfos ComponentInfos;
	hr = UniqueKey.MoreInformation(ComponentInfos);
	if (hr != S_OK)
	{
		return hr;
	}
	...
	CATAdpPLMAttributeSet AttrSet;
	hr = ComponentInfos.GetAttributeSet(spCATITypeOnPLMType, AttrSet);
	if (hr != S_OK)
	{
		return hr;
	}
	CATString strAttrName = iStrAttrName.ConvertToChar();
	CATIAdpPLMValueReader* pValueReader = NULL;
	AttrSet.GetAttributeValue(strAttrName, pValueReader);
	if (pValueReader == NULL)
	{
		return E_FAIL;
	}
	pValueReader->GetStringValue(oStrAttrValue);
	if (pValueReader != NULL)
	{
		pValueReader->Release();
		pValueReader = NULL;
	}
...
HRESULT CUSCAAUtilService::GetOccurrenceFromSystemInstance(const CATIPLMNavInstance_var & ihNavInst,
															...
															CATIPLMNavOccurrence_var & ohNavOcc)
{
	...
	CATIPLMNavReference* piRootRef = NULL;
	instancePath[1]->GetFather(piRootRef);
	CATIPLMNavOccurrence* ihRootOcc = NULL;
	HRESULT rc = hSystemOccAuth->GetOrCreateRootOccurrence(piRootRef, ihRootOcc);

	CATIPLMNavOccurrence* pNavOcc = NULL;
	rc = hSystemOccAuth->GetOccurrence(ihRootOcc, instancePath, pNavOcc);
	ohNavOcc = pNavOcc;
	if (NULL != pNavOcc)
	{
		pNavOcc->Release();
...
// 特征路径
void CUSCAAUtilService::PathElementString(CATPathElement * ipPath, CATUnicodeString & oPathName)
{
	oPathName = "";
	if (NULL != ipPath)
	{
...
CATUnicodeString CUSCAAUtilService::GetServerURIFromEKL(CATBaseUnknown_var spObject)
{
	cout << "  GetServerURIFromEKL begin! " << endl;

	CATUnicodeString returnValue = "";

	CATIParmPublisher_var spCATIParmPublisher = spObject;

	CATOmbLifeCycleRootsBag pBag;
	CATIKweModelServices_var spModelServices = CATCkeGlobalFunctions::GetModelServices();

	CATIParmPublisher_var spRelationSet = spModelServices->GetCurrentSetWithRepRef(CATIKweModelServices::Relation, spCATIParmPublisher, 1, (void*)&pBag);

	CATICkeRelationFactory_var spVolRelFactory = spRelationSet->GetFactory();

	CATICkeParmFactory_var spVolFactory = CATCkeGlobalFunctions::GetVolatileFactory();

	CATICkeFunction_var spFuncProgramGet;
	CATICkeRelationFactory_var spExpProgramGet;

	CATICkeParm_var spStringArg = spVolFactory->CreateString("Thisspace", "");

	//create script
	CATUnicodeString ScriptGet;
	//ScriptGet.Append("let Thisspace(String)\n");
	ScriptGet.Append("a1 = GetSystemInfo(\"serverurl\") \n");

	CATCkeListOf(Parm) iListOfParameters;
	CATCkeEvalContext *pEC = CATCkeGlobalFunctions::GetFunctionFactory()->CreateEvaluationContext(NULL_var);
	iListOfParameters.Append(spStringArg);

	cout << "  CreateProgram begin! " << endl;
	if (spVolRelFactory != NULL_var)
		spExpProgramGet = spVolRelFactory->CreateProgram("getServerURI", "", "", &iListOfParameters, ScriptGet, NULL_var, CATCke::False);
	else
		cout << "  CreateProgram failed! " << endl;
	spFuncProgramGet = spExpProgramGet;
	cout << "  CreateProgram begin! " << endl;
	if (!spFuncProgramGet)
		cout << "  spVolRelFactory is null  " << endl;
	else
		cout << "  spVolRelFactory create Program ok  " << endl;

	spFuncProgramGet->Run(&iListOfParameters, spStringArg, pEC);
	if (pEC)
	{
		delete pEC;
		pEC = NULL;
	...
	if (spStringArg != NULL_var)
	{
		CATICkeInst_var spInst = spStringArg->Value();
		returnValue = spInst->AsString();
	}
	cout << "  Server URL is ************  " << returnValue << endl;
