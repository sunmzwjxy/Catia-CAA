...
//===================================================================
// CAAReNameOperationsCmd.cpp
// Header definition of class CAAReNameOperationsCmd
//===================================================================
//
// Usage notes:
//
...
#include "CAAReNameOperationsCmd.h"
#include "CATCreateExternalObject.h"
CATCreateClass(CAAReNameOperationsCmd);
...
//-----------------------------------------------------------------------------
CAAReNameOperationsCmd::CAAReNameOperationsCmd() :
	CATStateCommand("CAAReNameOperationsCmd", CATDlgEngOneShot, CATCommandModeShared)
	, _OKAgent(NULL),
	_ApplyAgent(NULL),
	_CloseAgent(NULL)
{
	...
	_Panel = new CAAReNameOperationsDlg(pWnd, "Rename Operations");
	_Panel->Build();
...
void CAAReNameOperationsCmd::BuildGraph()
{
	...
	AddTransition(pSelectionState, NULL, AndCondition(IsOutputSetCondition(_OKAgent), Condition((ConditionMethod)&CAAReNameOperationsCmd::OKAction)), NULL);

	// Transition from Input state to NULL, when click on Cancel button
	AddTransition(pSelectionState, pSelectionState, IsOutputSetCondition(_ApplyAgent), Action((ActionMethod)&CAAReNameOperationsCmd::ApplyAction));

	// Transition from Input state to NULL, when click on Close button
	AddTransition(pSelectionState, NULL, IsOutputSetCondition(_CloseAgent), Action((ActionMethod)&CAAReNameOperationsCmd::CancelAction));

	AddTransition(pSelectionState, NULL, IsOutputSetCondition(_CancleAgent), Action((ActionMethod)&CAAReNameOperationsCmd::CancelAction));

	AddTransition(pSelectionState, pSelectionState, IsOutputSetCondition(_pSelectAgent), Action((ActionMethod)&CAAReNameOperationsCmd::ElementSelection, NULL, NULL, (void*)1));
	...
	//	(CATCommandMethod)&CAAReNameOperationsCmd::ChangeRadioNotifyCopy, NULL);
	...
	//	(CATCommandMethod)&CAAReNameOperationsCmd::ChangeRadioNotifyInput, NULL);
...
CATBoolean CAAReNameOperationsCmd::ElementSelection(void * data)
{
...
void CAAReNameOperationsCmd::FilterSelectObject()
{
...
CATBoolean CAAReNameOperationsCmd::ApplyAction(void *data)
{
	if (_ApplyAgent)
	{
...
CATBoolean CAAReNameOperationsCmd::OKAction(void *data)
{
	if (_OKAgent)
	{
...
CATBoolean CAAReNameOperationsCmd::CheckSelectObject()
{
...
CATBoolean CAAReNameOperationsCmd::CancelAction(void *data)
{
...
void CAAReNameOperationsCmd::ReNameOperations(CATIPLMNavReference *ipFO_OPFatherRef )
{
	HRESULT rc = E_FAIL;
...
void CAAReNameOperationsCmd::getOPtitles(CATIPLMNavReference * ipFO_OPFatherRef)
{
...
bool  CAAReNameOperationsCmd::GetOPPreAndSuffix(CATUnicodeString istrOPReftitle,int &onPreffix, int &onSuffix)
{
...
// 自然增加 计数 ，添加字母后缀
// 自然增加 计数 ，保持字母后缀
HRESULT CAAReNameOperationsCmd::SeekIndexToMod( CATListOfInt &onList_index_ChangePre,
...
// 自然增加 计数 ，添加字母后缀
// 自然增加 计数 ，保持字母后缀
HRESULT CAAReNameOperationsCmd::RunOPNameModify( CATListOfInt inList_index_ChangePre,
...
bool CAAReNameOperationsCmd::isNextContinue(CATListOfCATUnicodeString strList,int i)
...
// 1 检测出：是否 含有相同前缀，但后缀不是以A打头（不相邻！）的tmpStr —— 修改成 以A打头后缀！
HRESULT CAAReNameOperationsCmd::SearchModifyCase3_1(CATListOfCATUnicodeString & iostrListOPReftitle_tmp, CATListOfCATUnicodeString & iostrListOPInsttitle_tmp)			// io 变量！
{
	HRESULT rc = E_FAIL;
...
// 2 检测出：是否 带有后缀字母，但前缀字母唯一的 cese—— 修改成 去掉后缀，保留前缀！
HRESULT CAAReNameOperationsCmd::SearchModifyCase3_2(CATListOfCATUnicodeString & iostrListOPReftitle_tmp, CATListOfCATUnicodeString & iostrListOPInsttitle_tmp)			// io 变量！
{
	HRESULT rc = E_FAIL;
	int sizeofList = iostrListOPReftitle_tmp.Size();
	if (sizeofList == 0)
...
//// 0 检测出：是否 含有 相邻的前缀差值为10 ，跳过并行工序 tmpStr —— 修改成 前缀数字 差值为10！
//// 加减10 删后缀！____————————麻烦~~！
//HRESULT CAAReNameOperationsCmd::SearchModifyCase4(CATListOfCATUnicodeString & iostrListOPReftitle_tmp, CATListOfCATUnicodeString & iostrListOPInsttitle_tmp)			// io 变量！
//{
//	HRESULT rc = E_FAIL;
//	int sizeofList = iostrListOPReftitle_tmp.Size();
//	if (sizeofList == 0)
